...
const doRound = (
  ...
  if (remainTime > 0)
    if (isSelf) {
      ...
      const nextCandidate = [...currentSelf.toV.entries()]
      ...
      const nextCandidate = [...currentElephant.toV.entries()]
        .filter(([valve, travelTime]) => !openedValve.has(valve) && travelTime < remainTime)
        .map(([valve, timeTo]) => {
          const time = remainTime - timeTo - 1;
          const canRelease = time * valve.rate;
...
line$.pipe(
  ...
  console.log(
    'the most pressure you could release with you and an elephant working together',
    released,
  )
);