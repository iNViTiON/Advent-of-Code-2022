...
fromEvent(readable, 'readable').pipe(
  ...
  toArray(),
  mergeMap(grid => {
    const endRow = grid.findIndex(row => row.includes(-26));
    const endColumn = grid[endRow].findIndex(col => col === -26);
    grid[endRow][endColumn] = 26;
    return of(grid).pipe(
      switchMap(grid => nextLevel([endRow, endColumn], grid, new Map(), 0, { depth: 9999 })),
    );
  }),
  delay(0),
  ...
  console.log(
    'the fewest steps required to move starting from any square with elevation a to the location that should get the best signal',
    steps,
  )
);